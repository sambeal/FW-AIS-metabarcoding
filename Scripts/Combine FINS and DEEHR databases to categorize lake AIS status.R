...
deehr_df <- deehr_all_sf %>% st_drop_geometry() %>%
  mutate(Year = as.numeric(Year))

# Partial match function (all matches kept)
matched_list <- lapply(1:nrow(deehr_df), function(i){
  county <- deehr_df$County[i]
  name   <- deehr_df$Name[i]
  
  fins_sub <- fins_df %>% filter(County == county)
  
  matched <- fins_sub %>%
    filter(str_detect(Name, fixed(name, ignore_case = TRUE)) |
             str_detect(fixed(name, ignore_case = TRUE), Name))
  
  if(nrow(matched) == 0) return(NULL)
  
  deehr_info <- deehr_df[i, ] %>% select(Species.Name, Year)
...
################################################################################
# 4. SPATIAL ASSIGNMENT FOR UNMATCHED POINTS
################################################################################

# Identify unmatched points
matched_keys <- paste(deehr_matched_name$County, deehr_matched_name$Name)
deehr_unmatched <- deehr_all_sf %>%
  filter(!(paste(County, Name) %in% matched_keys))
...
if(nrow(deehr_unmatched) > 0){
  ...
  deehr_matched_spatial <- bind_cols(
    matched_lakes_sf %>% st_drop_geometry() %>% select(County, Name, Site.Code, Primary.Watershed, Watershed.Code),
    deehr_nearest_sf %>% st_drop_geometry() %>% select(Species.Name, Year)
  )
} else {
